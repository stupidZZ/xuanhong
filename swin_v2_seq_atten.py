...
def load_checkpoint_swin(model,
    ...
    torch.nn.Module.load_state_dict(model, state_dict, strict=strict)
    return checkpoint
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 ...
                 rpe_output_type='normal', attn_type='normal', mlpfp32=False, head_chunk_size=None):
    ...
    def forward(self, x, mask=None):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            q_split = torch.split(q.float(), self.num_heads // self.head_chunk_size, dim=1)
            k_split = torch.split(k.float(), self.num_heads // self.head_chunk_size, dim=1)
            logit_scale_split = torch.split(logit_scale.float(), self.num_heads // self.head_chunk_size, dim=0)
        v_split = torch.split(v, self.num_heads // self.head_chunk_size, dim=1)
        ...
        relative_position_bias_split = torch.split(relative_position_bias, self.num_heads // self.head_chunk_size,
                                                   dim=0)
        ...
        for i_c in range(self.head_chunk_size):
            if self.attn_type == 'cosine_mh':
                attn = (q_split[i_c] @ k_split[i_c].transpose(-2, -1)) * logit_scale_split[i_c].float()
            elif self.attn_type == 'normal':
                attn = (q_split[i_c] @ k_split[i_c].transpose(-2, -1))
            else:
                raise NotImplementedError()
            
            attn = attn + relative_position_bias_split[i_c].unsqueeze(0)

            if mask is not None:
                _nW = mask.shape[0]
                attn = attn.view(B_ // _nW, _nW, self.num_heads // self.head_chunk_size, N, N) + mask.unsqueeze(
                    1).unsqueeze(0)
                attn = attn.view(-1, self.num_heads // self.head_chunk_size, N, N)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 relative_coords_table_type='norm8_log', rpe_hidden_dim=512,
                 rpe_output_type='normal', attn_type='normal', mlpfp32_blocks=[-1],
                 head_chunk_size=None):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlockPost(dim=dim, input_resolution=input_resolution,
...
class SwinTransformerV2SeqAtten(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 head_chunk_size=[None, None, None, None],
                 **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                head_chunk_size=head_chunk_size[i_layer])
